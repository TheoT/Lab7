...
classdef smu < handle
    ...
    methods
        function obj = smu(varargin)
            ...
            usb(obj.INITIALIZE);
            obj.buffer = zeros(1, 64);
            obj.dev = usb(obj.OPEN_DEVICE, 26214, 43981, num);
            if obj.dev==-1
            ...
            elseif obj.dev==-3
                ...
                [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 0, 9, 1, 0, 0);
                if ret<0
        ...
        function autorange(obj, ch)
           if obj.dev>=0
                if (ch==1) || (ch==2)
                    ...
                    while done==0
                        [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_MEAS_KILL60HZ, 0, uint16(ch), 6);
                        if ret<0
                            fprintf('Unable to send GET_MEAS_KILL60HZ vendor request.\n');
                            done = 1;
                        else
                            val = 256*uint16(obj.buffer(2))+uint16(obj.buffer(1));
                            ref = 256*uint16(obj.buffer(4))+uint16(obj.buffer(3));
                            if (val>=ref)
                                res = val-ref;
                            else
                                res = ref-val;
                            end
                            range = uint8(obj.buffer(5));
                            fn = uint8(obj.buffer(6));
...
%%
        function autorange = get_autorange(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_AUTORANGE, 0, uint16(ch), 1);
                    if ret<0
                        fprintf('Unable to send GET_AUTORANGE vendor request.\n');
                    else
                        autorange = uint8(obj.buffer(1));
                    end
                else
...
%%
        function [src1str, meas1str, auto1str, src2str, meas2str, auto2str] = get_display(obj)
            if obj.dev>=0
                [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_DISPLAY, 0, 1, 24);
                if ret<0
                    fprintf('Unable to send GET_DISPLAY vendor request.\n');
                else
                    if uint8(obj.buffer(6))==obj.SRCV_MEASI
                        value = double(256*uint16(obj.buffer(2))+uint16(obj.buffer(1)))-double(256*uint16(obj.buffer(4))+uint16(obj.buffer(3)));
                        value = obj.get_src_vmult(uint8(obj.buffer(5))+1)*value;
                        src1str = sprintf(deblank(obj.get_disp_srcv_fmt(uint8(obj.buffer(5))+1, :)), value);
                        value = double(256*uint16(obj.buffer(8))+uint16(obj.buffer(7)))-double(256*uint16(obj.buffer(10))+uint16(obj.buffer(9)));
                        value = obj.get_meas_imult(uint8(obj.buffer(11))+1)*value;
                        meas1str = sprintf(deblank(obj.get_disp_measi_fmt(uint8(obj.buffer(11))+1, :)), obj.get_disp_imult(uint8(obj.buffer(11))+1)*value);
                    else
                        value = double(256*uint16(obj.buffer(2))+uint16(obj.buffer(1)))-double(256*uint16(obj.buffer(4))+uint16(obj.buffer(3)));
                        value = obj.get_src_imult(uint8(obj.buffer(5))+1)*value;
                        src1str = sprintf(deblank(obj.get_disp_srci_fmt(uint8(obj.buffer(5))+1, :)), obj.get_disp_imult(uint8(obj.buffer(5))+1)*value);
                        value = double(256*uint16(obj.buffer(8))+uint16(obj.buffer(7)))-double(256*uint16(obj.buffer(10))+uint16(obj.buffer(9)));
                        value = obj.get_meas_vmult(uint8(obj.buffer(11))+1)*value;
                        meas1str = sprintf(deblank(obj.get_disp_measv_fmt(uint8(obj.buffer(11))+1, :)), value);
                    end
                    if uint8(obj.buffer(12))==obj.ON
                        auto1str = 'AUTO';
                    else
                        auto1str = '    ';
                    end
                    if uint8(obj.buffer(18))==obj.SRCV_MEASI
                        value = double(256*uint16(obj.buffer(14))+uint16(obj.buffer(13)))-double(256*uint16(obj.buffer(16))+uint16(obj.buffer(15)));
                        value = obj.get_src_vmult(uint8(obj.buffer(17))+1)*value;
                        src2str = sprintf(deblank(obj.get_disp_srcv_fmt(uint8(obj.buffer(17))+1, :)), value);
                        value = double(256*uint16(obj.buffer(20))+uint16(obj.buffer(19)))-double(256*uint16(obj.buffer(22))+uint16(obj.buffer(21)));
                        value = obj.get_meas_imult(uint8(obj.buffer(23))+1)*value;
                        meas2str = sprintf(deblank(obj.get_disp_measi_fmt(uint8(obj.buffer(23))+1, :)), obj.get_disp_imult(uint8(obj.buffer(23))+1)*value);
                    else
                        value = double(256*uint16(obj.buffer(14))+uint16(obj.buffer(13)))-double(256*uint16(obj.buffer(16))+uint16(obj.buffer(15)));
                        value = obj.get_src_imult(uint8(obj.buffer(17))+1)*value;
                        src2str = sprintf(deblank(obj.get_disp_srci_fmt(uint8(obj.buffer(17))+1, :)), obj.get_disp_imult(uint8(obj.buffer(17))+1)*value);
                        value = double(256*uint16(obj.buffer(20))+uint16(obj.buffer(19)))-double(256*uint16(obj.buffer(22))+uint16(obj.buffer(21)));
                        value = obj.get_meas_vmult(uint8(obj.buffer(23))+1)*value;
                        meas2str = sprintf(deblank(obj.get_disp_measv_fmt(uint8(obj.buffer(23))+1, :)), value);
                    end
                    if uint8(obj.buffer(24))==obj.ON
                        auto2str = 'AUTO';
                    else
                        auto2str = '    ';
                    end
...
%%
        function fn = get_function(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_FN, 0, uint16(ch), 1);
                    if ret<0
                        fprintf('Unable to send GET_FN vendor request.\n');
                    else
                        fn = uint8(obj.buffer(1));
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
...
%%
        function range = get_irange(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_IRANGE, 0, uint16(ch), 1);
                    if ret<0
                        fprintf('Unable to send GET_IRANGE vendor request.\n');
                    else
                        range = uint8(obj.buffer(1));
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
            ...
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    ...
                    if ret<0
                        ...
                        value = double(256*uint16(obj.buffer(2))+uint16(obj.buffer(1)))-double(256*uint16(obj.buffer(4))+uint16(obj.buffer(3)));
                        if uint8(obj.buffer(6))==obj.SRCV_MEASI
                            value = value*obj.get_meas_imult(uint8(obj.buffer(5))+1);
                            units = 1;
                        else
                            value = value*obj.get_meas_vmult(uint8(obj.buffer(5))+1);
                            units = 0;
                        end
                    end
                else
...
%%
        function [value, units] = get_source(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_SRC, 0, uint16(ch), 6);
                    if ret<0
                        fprintf('Unable to send GET_SRC vendor request.\n');
                    else
                        value = double(256*uint16(obj.buffer(2))+uint16(obj.buffer(1)))-double(256*uint16(obj.buffer(4))+uint16(obj.buffer(3)));
                        if uint8(obj.buffer(6))==obj.SRCV_MEASI
                            value = value*obj.get_src_vmult(uint8(obj.buffer(5))+1);
                        else
                            value = value*obj.get_src_imult(uint8(obj.buffer(5))+1);
                        end
                        units = uint8(obj.buffer(6));
                    end
                else
                    fprintf('Illegal channel number specified.\n');
                end
...
%%
        function range = get_vrange(obj, ch)
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 192, obj.GET_VRANGE, 0, uint16(ch), 1);
                    if ret<0
            ...
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    if (arange<0) || (arange>1)
                        ...
                        [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 64, obj.SET_AUTORANGE, uint16(arange), uint16(ch), 0);
                        if ret<0
            ...
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    if (fn<0) || (fn>1)
                        ...
                        [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 64, obj.SET_FN, uint16(fn), uint16(ch), 0);
                        if ret<0
            ...
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    if (irange<0) || (irange>6)
                        ...
                        [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 64, obj.SET_IRANGE, uint16(irange), uint16(ch), 0);
                        if ret<0
                ...
                if (ch==1) || (ch==2)
                    if (units<0) || (units>1)
                        ...
                        if range~=obj.OVER_RANGE
                            ...
                            [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 64, obj.SET_SRC, temp, 256*uint16(units)+uint16(ch), 0);
                            if ret<0
            ...
            if obj.dev>=0
                if (ch==1) || (ch==2)
                    if (vrange<0) || (vrange>3)
                        ...
                        [ret, obj.buffer] = usb(obj.CONTROL_TRANSFER, obj.dev, 64, obj.SET_VRANGE, uint16(vrange), uint16(ch), 0);
                        if ret<0
